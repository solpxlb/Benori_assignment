...
This file is standing context for every Codex session on this repo. Read it before doing anything.
...
- **GDELT DOC 2.0**: endpoint `https://api.gdeltproject.org/api/v2/doc/doc`. Query syntax: quoted phrases, OR inside parentheses, space = implicit AND. Example assembled query:
  `("acquisition" OR "merger" OR "buyout" OR "stake") ("FMCG" OR "consumer goods" OR "food" OR "beverage")`
  Params: `mode=artlist&format=json&maxrecords=75&timespan=7d&sort=datedesc`. Responses are sometimes non-JSON (HTML error pages) — check content-type / wrap json parse. Articles live under the `articles` key. Dates are `seendate` in `YYYYMMDDTHHMMSSZ`. No snippets in artlist mode. Rate-limit politely: sleep 0.5s between queries.
